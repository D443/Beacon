...
public class MainActivity extends ListActivity {
	...
	private static String getCtx(){
		...
		return " Beacon bilgileri aþaðýdadýr:";
	}
		...
		public void onLeScan(final BluetoothDevice device,final int rssi,
			...
			runOnUiThread(new Runnable() {
				...
				public void run() {
					...
					if (rssi >=-35) a="Beacon sadece bir kaç santim uzaðýnýzda";
					else if (rssi>=-50 && rssi <=-35) a="Beaconla ayný oda içerisindesiniz ve yakýnlýðýnýz en fazla 1 metre çapýnda";
					else if (rssi>=-65 && rssi <=-45) a="Beacondan bir miktar uzaktasýnýz mesafe 2 metre civarý";
					else if (rssi>=-80 && rssi <=-65) a="Beaconla ayný odada olmama ihtimaliniz var";
					else if (rssi>=-90 && rssi <=-80)  a="Beacona uzaktasýnýz bulunduðunuz konumu birkaç metre deðiþtirip tekrar tarama yapýnýz";
					else if (rssi>=-100 && rssi <=-90) a="Beacondan alýnan en düþük sinyal aralýðýndasiniz açýk alanda 40 kapalý alanda 10-12 metre civarý bir uzaklýktasýnýz";
					
				
						String msg=getCtx()+
								
							"\nBulunan Cihaz:" +"Anahtarým"+
							"\nRssi:" + rssi+
					...
					/*		"\nTxPower:" +0+
							"\n Uzaklýðýn Yaklaþýk Formülü:"+"d = 10 ^ ((0 - rssi) / (10 * n))"+
